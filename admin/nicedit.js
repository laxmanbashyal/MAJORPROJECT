 ...
 * Updated for Red & White Blood Bank Theme
 * 
      ...
      if (bkLib.isMSIE) {
        document.write(
          '<style>.nicEdit-main p { margin: 0; }</style><script id=__ie_onload defer ' +
            (location.protocol == 'https:' ? "src='javascript:void(0)'" : 'src=//0') +
            '><\/script>'
        );
...
var nicEditorConfig = bkClass.extend({
  ...
  // Updated icon path - using a reliable CDN
  iconsPath: 'https://cdn.jsdelivr.net/npm/nicedit@0.9.0/nicEditIcons-latest.gif',
  buttonList: [
    'save',
    'bold',
    'italic',
...
var nicEditor = bkClass.extend({
  ...
  panelInstance: function (B, C) {
    ...
    var A = new bkElement('DIV')
      .setStyle({ width: (parseInt(B.getStyle('width')) || B.clientWidth) + 'px' })
      .appendBefore(B);
    this.setPanel(A);
  ...
  construct: function (G, D, C) {
    ...
    if (H || this.options.hasPanel) {
      var B =
        bkLib.isMSIE &&
        !(
          typeof document.body.style.maxHeight != 'undefined' && document.compatMode == 'CSS1Compat'
        );
      var E = {
        width: newX + 'px',
        border: '1px solid #D32F2F', // Red theme border
        borderTop: 0,
        overflowY: 'auto',
        overflowX: 'hidden',
      };
...
var nicEditorPanel = bkClass.extend({
  construct: function (E, B, A) {
    ...
    this.panelElm = new bkElement('DIV')
      .setStyle({ margin: '2px', marginTop: '0px', zoom: 1, overflow: 'hidden' })
      .addClass('panel')
...
var nicEditorButton = bkClass.extend({
  construct: function (D, A, C, B) {
    ...
    this.button = new bkElement('DIV')
      .setStyle({ width: '18px', height: '18px', overflow: 'hidden', zoom: 1, cursor: 'pointer' })
      .addClass('button')
  ...
  key: function (A, B) {
    if (
      this.options.key &&
      B.ctrlKey &&
      String.fromCharCode(B.keyCode || B.charCode).toLowerCase() == this.options.key
    ) {
      this.mouseClick();
      if (B.preventDefault) {
        B.preventDefault();
...
var nicEditorSelect = bkClass.extend({
  construct: function (D, A, C, B) {
    ...
    this.control = new bkElement('div')
      .setStyle({ overflow: 'hidden', float: 'right', height: '18px', width: '16px' })
  ...
  over: function (A) {
    A.setStyle({ backgroundColor: '#FFCDD2' });
  },
  out: function (A) {
    A.setStyle({ backgroundColor: '#ffffff' });
  },
  add: function (B, A) {
...
var nicEditorFontFormatSelect = nicEditorSelect.extend({
  ...
  init: function () {
    ...
    for (itm in this.sel) {
      ...
      this.add(
        '<' + A + '>',
        '<' + itm + ' style="padding: 0px; margin: 0px;">' + this.sel[itm] + '</' + A + '>'
      );
    }
  },
});
    ...
    if (!this.isDisabled) {
      if (this.pane && this.pane.pane) {
        ...
        this.pane = new nicEditorPane(
          this.contain,
          this.ne,
          { width: this.width || '270px', backgroundColor: '#fff', borderRadius: '8px' },
          this
        );
        this.addPane();
...
bkLib.onDomLoaded(function () {
  // Configure NicEdit with Red & White Theme
  new nicEditor({
    buttonList: [
      'fontSize',
      'fontFamily',
